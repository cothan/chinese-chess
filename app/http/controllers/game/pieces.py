...
class General(Piece):
    ...
    def successors(self, grid):
        ...
        for possibility in possibilities:
            ...
            if retval[0]:
                ret_pieces.append((General(
                    Position(possibility[0], possibility[1]), self.color, self.name), retval[1]))
    ...
    def successors(self, grid):
        ...
        if self.pos.x > 1:  # Left
            if not grid[self.pos.x-1][self.pos.y]:  # check left hobble
                possibilities.extend(
                    [(self.pos.x-2, self.pos.y-1), (self.pos.x-2, self.pos.y+1)])
        ...
        if self.pos.y > 1:  # Down
            if not grid[self.pos.x][self.pos.y-1]:  # check down hobble
                possibilities.extend(
                    [(self.pos.x-1, self.pos.y-2), (self.pos.x+1, self.pos.y-2)])
        if self.pos.y < 8:  # Up
        ...
        for possibility in possibilities:
            ...
            if retval[0]:
                ret_pieces.append(
                    (Horse(Position(possibility[0], possibility[1]), self.color, self.name), retval[1]))
    ...
    def successors(self, grid):
        ...
        for poss in move_poss:
            ...
            if retval[0] and not retval[1]:  # Didn't land on anything
                ret_pieces.append(
                    (Cannon(Position(poss[0], poss[1]), self.color, self.name), retval[1]))
...
class Soldier(Piece):
    ...
    def successors(self, grid):
        ...
        if self._across_river():
            if self.color:
                possibilities = [
                    (self.pos.x, self.pos.y+1), (self.pos.x-1, self.pos.y), (self.pos.x+1, self.pos.y)]
            else:
                possibilities = [
                    (self.pos.x, self.pos.y-1), (self.pos.x-1, self.pos.y), (self.pos.x+1, self.pos.y)]
        else:
